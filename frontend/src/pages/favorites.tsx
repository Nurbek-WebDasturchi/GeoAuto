...
export const FavoritesPage = () => {
  ...
  const removeFavorite = useFavoriteListing({ remove: true });
  return (
    <main className="container py-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {data?.data.map((listing) => <ListingCard key={listing.id} listing={listing} onFavorite={(id) => removeFavorite.mutate(id)} />)}
      </div>
